...
| `ui.lint` | `{}` | `{ findings: [{code, message}] }` (layout/contrast audits: W0103/W0104/W0105/W0301, **W0110** oversize sprite, **W0303** unreadable text contrast — APCA vs the composited backdrop, **W0111** effectively-transparent node, **W0112** node laid out entirely outside the window, **W0113** occluded interactive node, **W0114** blank frame, **W0115** renderer backend with known defects, **W0403** painted-truncated text, **W0116** stuck finite animation, **W0117** text-cache thrashing). In a `dev-observability` build these are also **pushed** to `app.logs` as they appear (O0.3) |
