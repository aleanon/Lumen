...
- *Still missing (A11Y2c, open)*: `route_at_action` handles `Action::Click` only, so AccessKit's `ScrollUp`/`ScrollDown`/`ScrollIntoView`/`SetScrollOffset`/`ScrollToPoint` — all present in 0.24.1 — are dropped. An AT can therefore drive the list only by synthesising key presses, not by asking the app to scroll. The keyboard half above makes the control operable; this would make it *directly* addressable.
