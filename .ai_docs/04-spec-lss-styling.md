...
failure**. Current split: **78 known, 58 applied, 20 parse-only** (PROP1's two mechanical layout batches landed 2026-08-08; the parity test's tripwire ratchets down with each one). A parse-only
...
| **parse-only** | `grid-*` (track lists unparsed), `overflow`, `filter`, `transform(-origin)`, `z-index`, `cursor`, `font-family/style/features/variation`, `letter-spacing`, `text-align/overflow/wrap/decoration`, `selection-color` |
