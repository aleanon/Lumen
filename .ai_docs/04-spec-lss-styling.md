...
failure**. Current split: **78 known, 67 applied, 11 parse-only** — no layout property is parse-only, and of typography only `font-features`/`font-variation`/`text-overflow`/`text-wrap`/`text-decoration` remain (PROP1's two mechanical layout batches landed 2026-08-08; the parity test's tripwire ratchets down with each one). A parse-only
...
| **parse-only** | `filter`, `transform(-origin)`, `z-index`, `cursor`, `font-features/variation`, `text-overflow/wrap/decoration`, `selection-color` |
