...
- **Workspace:** virtual manifest at root listing all 11 crates + `lumen` facade (`02 §1`), `examples/`, `benches/`. Lockstep version (`0.3.0` as of 2026-09-01 — minor bump for the MUT/E series merge; 0.x treats minor as the breaking slot). **What breaks in 0.3.0:** `#[derive(Reactive)]`'s generated signal accessor is renamed `field()` → `field_signal()` (the plain name is now the MUT8 instance read), and the derive emits `::lumen` paths by default, so a caller that does not depend on the facade needs `#[reactive(crate = "lumen_core")]`; `Stack` bodies take `FnOnce` instead of `FnMut`; ten single-widget example crates were removed (E2a — `widget_showcase` covers them); taffy resolves 0.14. **Additive:** `App::with_state`, `App::view`, `Stack::{align_items, justify_content, centered, shadow, grow, width, height}`, `Element::bind_text_color`, `Runtime::{install_state, with_state, take_written}`, public `SignalId`. `publish = false` for now. The lockstep number is read dynamically by `just run` (`-p <name>@<version>`), so it follows the bump with no edit; the pin exists to disambiguate an example crate from a same-named dependency. Shared `[workspace.dependencies]` table so every whitelisted crate version is pinned in exactly one place (satisfies ADR-003 "pin minor versions at repo init").
