...
/// `transform` composes into `PushLayer`'s existing `Affine`, so it is a bridge
/// rather than a new render pass. Asserted on pixels — a transform changes
/// where things are drawn, never the layout box, so bounds cannot witness it.
#[test]
fn transform_changes_the_frame_but_not_the_layout_box() {
    ...
    let (plain, plain_box) = probe("#a { font-size: 24px; }");
    let (scaled, scaled_box) = probe("#a { font-size: 24px; transform: scale(2); }");
    assert_ne!(plain, scaled, "scale(2) must change the frame");
    assert_eq!(
        plain_box, scaled_box,
        "transform must NOT move the layout box — it is a paint-time effect"
    );
    // `none` is the identity, so it must equal saying nothing.
    assert_eq!(probe("#a { font-size: 24px; transform: none; }").0, plain);
}

/// `transform-origin` re-anchors the matrix. Rotating about the centre (the CSS
/// default) and about the top-left must produce different frames — without the
/// re-anchoring, a rotate would swing the node around the window origin.
#[test]
fn transform_origin_re_anchors_the_matrix() {
    fn frame(lss: &str) -> Vec<u8> {
        let mut h = App::new(|_cx: &mut BuildCx| -> Element {
            widgets::column(vec![widgets::text("xx").id("a")]).id("root")
        })
    ...
    let centre = frame("#a { font-size: 24px; transform: rotate(30deg); }");
    let corner =
        frame("#a { font-size: 24px; transform: rotate(30deg); transform-origin: left top; }");
    assert_ne!(centre, corner, "transform-origin must change the result");
}

/// A px `translate` works; a PERCENTAGE one is rejected, because a percentage
/// there is relative to the node's own box, which the style engine cannot see.
/// Misinterpreting it as px would be a silent wrong answer.
#[test]
fn percentage_translate_is_rejected() {
    let (_, ds) = lumen_style::parse("t.lss", "x { transform: translate(50%); }");
    assert!(
        ds.iter().any(|d| d.code == lumen_core::codes::W0109),
