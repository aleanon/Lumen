...
| **Perf regression** | Rebuild where a patch was expected; text churn (7 ms full CPU raster); non-virtualized list. (Hover rebuilds are scope-memoized since A.1; a restyle-only hover path lands with A.5) | `ui.lastChange` after the interaction: `rebuild` vs `patch` vs `idle`; `cargo bench -p lumen-benches` + `scripts/perf_gate.sh` |
