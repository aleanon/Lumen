...
| **Perf regression** | Rebuild where a patch was expected; text churn (7 ms full CPU raster); non-virtualized list. (Hover/focus/pressed are restyle-only since A.5a — `ui.lastChange` says `restyle`, not `rebuild`; a hover that *rebuilds* means a `:hovered` layout/typography rule escalated, which is expected) | `ui.lastChange` after the interaction: `rebuild` vs `patch` vs `idle`; `cargo bench -p lumen-benches` + `scripts/perf_gate.sh` |
