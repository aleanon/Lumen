...
- **Reactive text is the default — write the binding, not the value.** Every
  ...
  **~93 µs bound against ~832 µs** for the same change written as a value read
  ...
  patch. The fallback is scoped (MUT1): only the scopes enclosing the moving
  node re-run, every other scope splices, and patchable sibling bindings in
  the same pump still commit — one growing label costs one scope's rebuild,
  not the view's. A stretched single-line label under a definite container (the common
...
| `motion` | `spring(cx,…)`, `shared_bounds(cx, name, target, ms)`, `route_progress(cx, name, route, ms)`, `drag_surface`/`drag_fraction` | store-backed, virtual-clock; morph on target change, per-navigation progress, gesture→fraction (M.3) |
